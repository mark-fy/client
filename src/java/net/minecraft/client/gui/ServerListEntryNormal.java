...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
        if (!this.server.field_78841_f) {
            ...
            field_148302_b.submit(() -> {
                try {
                    this.owner.getOldServerPinger().ping(ServerListEntryNormal.this.server);
                } catch (UnknownHostException unknownHostException) {
                    handleConnectionError("Can't resolve hostname");
                } catch (Exception exception) {
                    handleConnectionError("Can't connect to server.");
                }
            });
        }

        boolean isVersion47 = this.server.version > 47;
        boolean isVersion47OrBelow = this.server.version < 47;
        boolean showVersionInfo = isVersion47 || isVersion47OrBelow;

        this.mc.fontRenderer.drawString(this.server.serverName, x + 32 + 3, y + 1, 16777215);
        List<String> motdLines = this.mc.fontRenderer.listFormattedStringToWidth(this.server.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(motdLines.size(), 2); ++i) {
            this.mc.fontRenderer.drawString(motdLines.get(i), x + 32 + 3, y + 12 + this.mc.fontRenderer.FONT_HEIGHT * i, 8421504);
        }

        String s2 = showVersionInfo ? EnumChatFormatting.DARK_RED + this.server.gameVersion : this.server.populationInfo;
        ...
        if (showVersionInfo) {
            l = 5;
            s1 = isVersion47 ? "Client out of date!" : "Server out of date!";
    ...
    private void handleConnectionError(String message) {
        ...
        this.server.serverMOTD = EnumChatFormatting.DARK_RED + message;
    }
    ...
    private boolean func_178013_b() { return true; }

    private void prepareServerIcon() {
        String base64IconData = this.server.getBase64EncodedIconData();

        if (base64IconData == null) {
        ...
        try {
            BufferedImage bufferedImage = decodeBase64ToImage(base64IconData);

            if (isValidServerIcon(bufferedImage)) {
                createOrUpdateServerIconTexture(bufferedImage);
            } else {
                mc.getTextureManager().deleteTexture(this.serverIcon);
                field_148305_h = null;
            }
    ...
    private void createOrUpdateServerIconTexture(BufferedImage image) {
        if (field_148305_h == null) {
            field_148305_h = new DynamicTexture(image.getWidth(), image.getHeight());
            mc.getTextureManager().loadTexture(this.serverIcon, this.field_148305_h);
        }

        image.getRGB(0, 0, image.getWidth(), image.getHeight(), field_148305_h.getTextureData(), 0, image.getWidth());
        field_148305_h.updateDynamicTexture();
    }

    private BufferedImage decodeBase64ToImage(String base64Data) throws IOException {
        ByteBuf byteBuf = Unpooled.copiedBuffer(base64Data, Charsets.UTF_8);
        ByteBuf decodedData = Base64.decode(byteBuf);

        try {
            return TextureUtil.readBufferedImage(new ByteBufInputStream(decodedData));
        } finally {
            byteBuf.release();
            decodedData.release();
        }
    ...
    private boolean isValidServerIcon(BufferedImage image) {
    ...
    public ServerData getServerData() { return this.server; }
}
