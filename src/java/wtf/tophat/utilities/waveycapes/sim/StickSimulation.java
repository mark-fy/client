package wtf.tophat.utilities.waveycapes.sim;
...
//		this.gravity = WaveyCapesBase.config.gravity;
		...
		this.gravity = 32; // gravity config value.
		...
		for (i = this.points.size() - 2; i >= 1; i--) {
			double angle = getAngle(((Point) this.points.get(i)).position, ((Point) this.points.get(i - 1)).position,
					((Point) this.points.get(i + 1)).position);
			angle *= 57.2958D;
			if (angle > 360.0D)
				angle -= 360.0D;
			if (angle < -360.0D)
			...
			if (abs < (180.0F - this.maxBend)) {
				Vector2 replacement = getReplacement(((Point) this.points.get(i)).position,
						((Point) this.points.get(i - 1)).position, angle, (180.0F - this.maxBend + 1.0F));
				((Point) this.points.get(i + 1)).position = replacement;
			}
			if (abs > (180.0F + this.maxBend)) {
				Vector2 replacement = getReplacement(((Point) this.points.get(i)).position,
						((Point) this.points.get(i - 1)).position, angle, (180.0F + this.maxBend - 1.0F));
