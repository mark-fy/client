...
public class DropDownGUI extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (Module.Category category : Module.Category.values()) {
            ...
            double minX = x, maxX = x + categoryWidth, maxY = y + categoryHeight;

            DrawingUtil.rectangle(minX, y, maxX - minX, maxY - y, false, CategoryUtil.getCategoryColor(category));
                    ...
                    if (DrawingUtil.hovered((float) mouseX, (float) mouseY, (float) modX, (float) modY, (float) categoryWidth, 20)) {
                        if (mouseButton == 0) {
                            ...
                            if (TopHat.settingManager.getSettingsByModule(module).size() > 0) {
                                mc.displayGuiScreen(new DropDownSetting(this, module));
                            }
                        } else {
