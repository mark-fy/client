...
import wtf.tophat.client.utilities.world.BlockUtil;
...
public class CustomPathFinder {
        ...
        block0: for (int i2 = 0; i2 < loops; ++i2) {
            ...
            for (Hub hub : new ArrayList<Hub>(this.hubsToWork)) {
                ...
                for (Vec3 direction : flatCardinalDirections) {
                    ...
                    if (CustomPathFinder.checkPositionValidity(loc, false) && this.addHub(hub, loc, 0.0)) break block0;
                }
                Vec3 loc1 = BlockUtil.floorVec3(hub.getLoc().addVector(0.0, 1.0, 0.0));
                if ((!CustomPathFinder.checkPositionValidity(loc1, false) || !this.addHub(hub, loc1, 0.0)) && (!CustomPathFinder.checkPositionValidity(loc2 = BlockUtil.floorVec3(hub.getLoc().addVector(0.0, -1.0, 0.0)), false) || !this.addHub(hub, loc2, 0.0))) continue;
                break block0;
            }
        }
        if (this.nearest) {
    ...
    public static boolean checkPositionValidity(Vec3 loc, boolean checkGround) {
        return CustomPathFinder.checkPositionValidity((int)loc.xCoord, (int)loc.yCoord, (int)loc.zCoord, checkGround);
    }
    ...
    public static boolean checkPositionValidity(int x2, int y2, int z2, boolean checkGround) {
        ...
        return !CustomPathFinder.isBlockSolid(block1) && !CustomPathFinder.isBlockSolid(block2) && (CustomPathFinder.isBlockSolid(block3) || !checkGround) && CustomPathFinder.isSafeToWalkOn(block3);
    }
